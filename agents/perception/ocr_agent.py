...
async def run_ocr(state: VigilState) -> dict:
    ...
    try:
        ...
        duration = int((datetime.utcnow() - start).total_seconds() * 1000)
        logger.info("agent_complete", agent="ocr_agent", duration_ms=duration, procedures=len(result.procedures))
...
def _demo_clean() -> OCRResult:
    ...
    return OCRResult(
        ...
        procedures=[
            ...
            Procedure(code="02202", description="Radiographs, periapical, 2 films", fee_charged=44.00),
            Procedure(code="11111", description="Scaling, first unit", fee_charged=57.00),
        ],
        subtotal=183.00,
        tax=None,
        total=183.00,
